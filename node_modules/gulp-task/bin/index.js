...
(function() {
  var Promise, executeTask, foreach, gcallback, gulp, humanizeTime, isPromise, isStream, streamToPromise, tag, _gulp, _task, _tasks;
  ...
  streamToPromise = function(stream) {
    return new Promise(function(resolve) {
      return stream.pipe(gcallback(function() {
  ...
  _task = function(name, cb) {
    ...
    if (_gulp != null) {
      return _gulp.task(name, function() {
        return _task.run(name).then(_task.gulpTaskCallback);
      });
    }
  };
    ...
    if (_gulp == null) {
      for (name in _tasks) {
        ...
        _gulp.task(name, function() {
          return _task.run(name).then(gulpTaskCallback);
        });
      }
    }
    return _gulp = gulp;
  ...
  _task.watch = function(glob, options, cb) {
    if (options == null) {
      options = {};
    }
    ...
    gulp.watch(glob, options, function(event) {
      cb(gulp.src(event.path), event.path, event);
      return null;
    });
    return {
      pipe: function() {
        throw new Error('task.watch no longer returns a stream. Functionality has moved from the gulp-watch package to the gulp.watch function in order to avoid segmentation faults for large projects. If your project is small and you wish the old functionality, lock your version number down to 1.0.0');
      }
    };
  };
  ...
  _task.gulpTaskCallback = function() {};
